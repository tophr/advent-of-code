...
const seatLayout = `LLLLL.LLLLLL.LLLLLLLLL.LLL.LL.LLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLLLLL.LLLLLLLLLL.LLLLLLL
    ...
    row.forEach((seat, j) => {
      if ( seat === '.' ) {
        ...
        if( seats[i - 1] !== undefined) {
          if( seats[i - 1][j - 1 ] !== undefined) {
            adjSeats.push( seats[i - 1][j - 1] );
          }
          if( seats[i - 1][j] !== undefined) {
            adjSeats.push( seats[i - 1][j] );
          }
          if( seats[i - 1][j + 1] !== undefined) {
            adjSeats.push( seats[i - 1][j + 1] );
          }
        ...
        // Current row
        if( seats[i][j - 1] !== undefined) {
          adjSeats.push( seats[i][j - 1] );
        }
        if( seats[i][j + 1] !== undefined) {
          adjSeats.push( seats[i][j + 1] );
        }
...
function layoutIterator( layout ) {
  let layoutNew = seatFill(layout);
  if (  JSON.stringify(layout) !== JSON.stringify(layoutNew) ) {
    layoutIterator(layoutNew);
  } else {
