...
var textByLine = passwords.split("\n");
var passwordMulti = [];

textByLine.forEach( (password) => {
  passwordMulti.push( password.split(" ") );
});

function evalPassword( passwordArray ) {
  ...
  if ( charOccur >= range[0] && charOccur <= range[1]) {
    return true;
  }
}

var validPasswordTotal = 0;

passwordMulti.forEach( ( passArray ) => {
  if ( evalPassword( passArray ) ) {
    validPasswordTotal++;
  }
});
