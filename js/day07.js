const luggageEx = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
luggageArray.forEach((rule, i) => {
  ...
  const contains = pieces[1].split(', ');
  contains.forEach((subRule, i) => {
    if (subRule === 'no other bags.') {
      contains[i] = '';
    } else {
      let num = subRule.substr(0, 1);
      let color = subRule.substr(2).replace('.', '').replace(' bags', '').replace(' bag', '');
      contains[i] = [ num, color ];
    }
  });
  luggageArray[i] = [ pieces[0], contains];
});
// console.log(luggageArray);

let bagHolders = [];
function bagLookup( bagColor, bagList ) {
  let bagCounter = 0;

  bagList.forEach((rule, i) => {

    rule[1].forEach((subRule, i2) => {
      if (subRule[1] === bagColor ) {
        bagHolders.push(rule[0]);
        bagCounter++;
      }
    });
  });
