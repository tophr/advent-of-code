const cubesEx = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`;

const cubes = `Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
const gamesArray = cubes.split("\n");

let filteredGames = [];
let cubeCounts = [];

gamesArray.forEach((game, i) => {
  // Split game on colon space and return only the second half
  let gameArray = game.split(": ")[1];
  let handfuls = gameArray.split("; ");

  filteredGames[i] = handfuls;
  ...
  handfuls.forEach((handful, j) => {
    ...
    colors.forEach((color, k) => {
      ...
      // If value is larger, replace it
      if (cubeCounts[i][colorName] < parseInt(colorCount)) {
        cubeCounts[i][colorName] = parseInt(colorCount);
      }
...
console.log({cubeCounts});

function findPossibleGames( games, red, green, blue ) {
  let possibleGames = [];

  games.forEach((game, i) => {
    let possible = true;
    let gameCubeCounts = game;
    console.log(gameCubeCounts);
    // let gameCubeColors = ["red", "green", "blue"];
    
    if (parseInt(gameCubeCounts["red"]) > parseInt(red)) {
      console.log(gameCubeCounts["red"] + " " + parseInt(red));
      possible = false;
    } 
    if (gameCubeCounts["green"] > parseInt(green)) {
      possible = false;
      console.log("green");
    }
    if (gameCubeCounts["blue"] > parseInt(blue)) {
      possible = false;
      console.log("blue");
    }
